...
class SubsetWithTargets(torch.utils.data.Dataset):
...
def load_trainset(name, transform=None, train=True, path="./data/"):
    ...
    if _name == "cifar10":
        ...
        # Define the number of samples per class and total number of classes
        samples_per_class = 100
        num_classes = 2
        # Create a list to store indices of samples for each class
        np.random.seed(0)
        indices = []
        for i in range(num_classes):
            # Get indices of all samples of a particular class
            class_indices = np.where(np.array(trainset.targets) == i)[0]
            # Randomly select the required number of samples
            indices.extend(np.random.choice(class_indices, samples_per_class, replace=False))
        # Create a subset of the dataset using the selected indices
        # trainset = Subset(trainset, indices)
        # Use the custom class to create a subset
        trainset = SubsetWithTargets(trainset, indices)
        trainset.num_classes = 2
    elif _name == "cifar100":
...
def get_features(net, trainloader, verbose=True):
    ...
    for step, (batch_imgs, batch_lbls) in enumerate(train_bar):
        batch_features = net(batch_imgs.cuda(), heatmap_vis=True)
