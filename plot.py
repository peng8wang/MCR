...
def plot_heatmap(args, features, labels, epoch):
    ...
    features_sort, _ = utils.sort_dataset(features.numpy(), labels.numpy(), 
                            num_classes=num_classes, stack=False)
    features_sort_unnorm = np.vstack(features_sort)
    # features_sort_ = features_sort_unnorm / np.linalg.norm(features_sort_unnorm, axis=0)
    features_sort_ = features_sort_unnorm * 1.0

    num_samples_plot = len(features_sort) * features_sort[0].shape[0]
    print(num_samples_plot)
    ...
    ax.set_xticks(np.linspace(0, num_samples_plot, 6))
    ax.set_yticks(np.linspace(0, num_samples_plot, 6))
