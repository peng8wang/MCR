...
class ResNetMNIST(nn.Module):
    def __init__(self, block, num_blocks, feature_dim=512):
        ...
        self.reshape = torch.nn.Sequential(
            # nn.Linear(512 * block.expansion, 1024, bias=False),
            nn.Linear(784, 2048, bias=False),
            # nn.BatchNorm1d(1024),
    ...
    def forward(self, x, heatmap_vis=False):
        out = x.view(x.size(0), -1)
        out = self.reshape(out)
        if heatmap_vis == False:
            return out
        else:
            print('normalize output of the network')
